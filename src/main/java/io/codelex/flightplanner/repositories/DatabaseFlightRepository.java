...
public interface DatabaseFlightRepository extends JpaRepository<Flight, Long> {
    ...
    @Query("SELECT f FROM Flight f WHERE f.from.airport = :fromAirport AND f.to.airport = :toAirport AND f.departureTime BETWEEN :startOfDay AND :endOfDay")
    List<Flight> findFlightsByFromAirportAndToAirportAndDepartureTime(@Param("fromAirport") String fromAirport, @Param("toAirport") String toAirport, @Param("startOfDay") LocalDateTime startOfDay, @Param("endOfDay") LocalDateTime endOfDay);
}
